...
#TIMELIMIT  = "60minutes"      # Max walltime
...
#			print max(file_list)
			...
			if(FILENO == "000"):
				print "First run: ", FILENO
				add_job(WORKFLOW, DATA_SOURCE_DIR, FILENAME, FORMATTED_RUN, FILENO)

			if(FILENAME == max(file_list)):
				print "Last run: ", FILENO
				add_job(WORKFLOW, DATA_SOURCE_DIR, FILENAME, FORMATTED_RUN, FILENO)
			
#			if(FILENAME == file_list[len(file_list)/2]):
#				print "Mid Run:" FILENO
#				add_job(WORKFLOW, DATA_SOURCE_DIR, FILENAME, FORMATTED_RUN, FILENO)
